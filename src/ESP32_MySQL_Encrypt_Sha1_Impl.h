...
/**************************** 
  ESP32_MySQL_Encrypt_Sha1_Impl.h
  by Syafiqlim @ syafiqlimx
*****************************/
#pragma once

#ifndef ESP32_MYSQL_ENCRYPT_SHA1_IMPL_H
#define ESP32_MYSQL_ENCRYPT_SHA1_IMPL_H

#include <string.h>

#define ESP32_MYSQL_SHA1_K0     0x5a827999
#define ESP32_MYSQL_SHA1_K20    0x6ed9eba1
#define ESP32_MYSQL_SHA1_K40    0x8f1bbcdc
#define ESP32_MYSQL_SHA1_K60    0xca62c1d6
...
void Encrypt_SHA1::hashBlock() 
{
  ...
  for (i = 0; i < 80; i++) 
  {
    ...
    if (i < 20) 
    {
      t = (d ^ (b & (c ^ d))) + ESP32_MYSQL_SHA1_K0;
    } 
    else if (i < 40) 
    {
      t = (b ^ c ^ d) + ESP32_MYSQL_SHA1_K20;
    } 
    else if (i < 60) 
    {
      t = ((b & c) | (d & (b | c))) + ESP32_MYSQL_SHA1_K40;
    } 
    else 
    {
      t = (b ^ c ^ d) + ESP32_MYSQL_SHA1_K60;
    }
...
#endif    // ESP32_MYSQL_ENCRYPT_SHA1_IMPL_H
